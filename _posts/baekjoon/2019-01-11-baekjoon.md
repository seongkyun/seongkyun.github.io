...
- 점화식을 세워서 풀었다.
  - 육각형의 4시방향 숫자는 1, 7, 19, ... 이므로, $a_{n+1}=6\mult(n-1)+a_{n}$ 의 관계를 보인다.
  - 계차수열 $b_{n}$은 $b_{n}=a_{n+1}-a_{n}=6n$ 으로 정의된다.
  - 따라서,
    - $a_{n}=a_{1}+\sum^{n-1}_{k=1}b_{k}=1+\sum^{n-1}_{k=1}6k \\ =1+6\sum^{n-1}_{k=1}k \\ 1+6\frac{(n-1)\mult n}{2}$
    - $a_{n} = 1+3(n-1)\mult n \\ = 3n^{2}-3n+1$
